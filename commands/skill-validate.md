...
- Run deterministic validation from the repository root: `python3 -m tools.skill_valid <target>`.
- Do not run live/model-backed validation by default.
- Run live validation only when the user explicitly opts in: `python3 -m tools.skill_valid <target> --allow-live --harness <pi|kilo>`. Select the current supported harness; do not assume Pi is available.
