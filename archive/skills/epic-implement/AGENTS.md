...
Maintains Pi `/epic-implement` orchestration: parent-controlled beads epic implementation through formal gates, durable state, strict closure ownership, and recovery behavior. Preserve sequential current-session fallback when optional runners are unavailable.
