...
- owner-scoped versus repo-scoped semantics; verify that an owner-level value is never used as a proxy for a
  repo-level value without confirming all repos have been covered
...
Do not apply the Fallback Path Enumeration, Stateful Code, or Mirrored Implementation triggers unless directly
relevant to a data safety finding you already verified.
