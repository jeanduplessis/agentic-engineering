...
3. If a code comment or variable name claims a single reason for the fallback (e.g., "clean repos have no findings"), treat that as a hypothesis and actively search for counterexamples — other real-world states that produce the same triggering value but for which the fallback is incorrect.
...
Do not apply the Schema And Rollout Trigger or Mirrored Implementation Trigger unless directly relevant to a logic or error-handling finding you already verified.
