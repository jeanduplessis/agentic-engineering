...
- **Trust boundary violations** — User-controlled data crossing trust boundaries without validation (e.g.,
  client-supplied IDs used for server-side lookup without ownership check)
...
Expand only around verified security findings. Do not apply the Fallback Path Enumeration, Stateful Code,
Schema And Rollout, or Mirrored Implementation triggers unless they are directly relevant to a security
finding you already verified.
