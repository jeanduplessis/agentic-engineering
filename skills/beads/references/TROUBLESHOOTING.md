...
Prefer diagnosis and reversible actions. Ask before destructive repairs, setup changes, database reinitialization, migration, deletion, or low-level storage operations.
...
| No beads database | If the user requested setup, initialize carefully; otherwise ask before changing the project. |
...
| Shell quoting breaks text | Use comments, `--body-file`, `--stdin`, or a temporary file where the command supports it. |
...
Repair modes can mutate project state. Ask the user before running commands with flags such as `--fix`, `--clean`, `--yes`, `--force`, or migration-specific options. Prefer `--dry-run` when available.
...
Most agent task tracking does not require inspecting storage internals. Only investigate backend or collaboration state when the user asks or when bd itself reports a storage error that blocks the requested task.
...
If you cannot resolve a beads problem in-session, leave a useful comment or note in the current issue if writes still work. If writes do not work, report the diagnostics above in the conversation and avoid speculative repair.
