...
Maintain `SKILL.md` as the runtime contract for test-driven development using thin red-green-refactor vertical slices and beads (`bd`) task tracking for durable work.
...
Behavior evals are declared in `evals/manifest.json`; cases live in `evals/evals.json`. The evals check that the skill uses current `bd` command syntax, skips beads for tiny same-session changes, avoids silent setup, and preserves vertical-slice TDD behavior.
...
- Keep beads task commands aligned with the installed `bd --help` and `skills/beads/SKILL.md`.
- Do not add silent `bd init`, repair, sync, push, or storage mutation steps.
