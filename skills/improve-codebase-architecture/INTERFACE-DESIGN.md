...
Spawn 3+ `scout` children in parallel with `subagent({ workflowScript })` and `runs.all`. Each must produce a **radically different** interface for the deepened module.
