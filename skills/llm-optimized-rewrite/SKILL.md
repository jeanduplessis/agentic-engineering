...
changing meaning, constraints, values, ordering, relationships, compliance obligations, schemas, APIs, field
semantics, format contracts, or functional roles.
...
- Preserve all requirements, constraints, values, ordering, relationships, schemas, APIs, field semantics,
  compliance obligations, safety obligations, trigger behavior, file paths, file types, and format/style
  contracts.

- Never add, remove, weaken, reinterpret, or reorder requirements unless the user explicitly asks.

- Treat explicit qualifiers (`exact`, `strict`, `required`, `only`, `all`, `any`, `must`, `should`,
  `carefully`) as constraints unless equivalent wording preserves force.
