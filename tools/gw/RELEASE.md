...
`gw` releases are built and installed from this monorepo. There is currently no tag-based, GitHub Release, or package-registry workflow.

## Requirements

- Run releases on macOS with the Rust toolchain from `rust-toolchain.toml`.
- Start from a release branch based on the current local `main` branch.
- Keep the `main` worktree clean before merging.
- Use semantic versioning. Update `Cargo.toml`, the `gw` package entry in `Cargo.lock`, and `CHANGELOG.md` to the same version.
...
Run package commands from `tools/gw` unless a step says otherwise.
...
2. Choose the next semantic version and update:
   ...
   - `Cargo.lock`: the `gw` package version
...
4. Review and commit only the intended release files:
   ...
   git commit -m "feat(gw): release X.Y.Z"
   ```
