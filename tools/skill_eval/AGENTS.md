...
This directory contains the repo-level skill evaluation framework. Keep it small, deterministic by default, and explicit about whether results are synthetic or real.

## Design intent

- Run skill-owned eval manifests through one central runner.
- Capture enough evidence for human review: raw output, response, timing, usage placeholders, metadata, artifacts, workspace diff, events, grade, benchmark, and report.
...
- The only real harness adapter currently implemented is Pi (`harness: "pi"`). Live Pi execution must be gated by `allow_live: true` or `SKILL_EVAL_ALLOW_LIVE_PI=1`.
...
- Relative manifest paths are resolved from the manifest directory, including skill paths, custom graders, legacy eval files, and copy fixtures. If promotion writes a manifest to a different directory, rewrite those relative paths for the new location.
