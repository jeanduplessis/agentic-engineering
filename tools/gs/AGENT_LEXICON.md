...
| **Section count** | Number of entries in a section shown in the section header, e.g. `Staged (2)`. | Defining the entry count portion of section header content. | line total, diff total, file count when entries can duplicate a partially staged path |
| **Section stats** | Aggregate changed-line indicator rendered in the section header after the Section count, e.g. `Staged (2) +3/-1`; renders as Unknown stats when any Entry stats in that section are unknown. | Referring to per-section addition/deletion totals in section headers. | Section count, file count, per-entry stats |
| **Section divider** | Muted dashed horizontal `╌` line rendered under each visible Section header before its entries. | Separating Section stats from Entry stats in the Enhanced status view. | Border line unless referring to the repository context frame |
...
| **Stats separator** | The `/` between additions/deletions inside Entry stats or Section stats; vertically aligned and muted gray in colored output. | Discussing alignment or styling of the separator in `+N/-N` or `+?/-?`. | slash unless meaning generic punctuation |
| **Known text stats** | Stats with concrete additions/deletions for text changes, rendered `+N/-N`. | Describing text diff output. | changed lines when deletion/addition split matters |
| **Unknown stats** | Stats for binary or otherwise unknown line changes, rendered `+?/-?`. | Handling binary, submodule, or non-line-oriented changes. | binary, no stats, `+0/-0` |
...
- Use **Section count** for entry counts only; use **Section stats** for aggregate additions/deletions in section headers.
...
- Render **Section stats** as **Unknown stats** when any **Entry stats** in that section are **Unknown stats**.
...
- Align **Branch commit stats** with **Section stats** and **Entry stats** when sections are visible.
- Vertically align the **Stats separator** across visible **Section** headers and **Entry** rows.
- Render one blank line between the repository context frame and the first visible **Section**.
- Render one **Section divider** directly below each visible **Section** header.
...
- A visible **Section** header contains one **Section count** and one **Section stats** value.
- A **Section divider** separates each visible **Section** header from its **Entry** values.
- An **Entry** contains one **Git-letter status symbol**, one **Display path**, and one **Entry stats** value.
- **Entry stats** and **Section stats** are either **Known text stats** or **Unknown stats** and contain one **Stats separator**.
...
| changed lines | Can imply total changes or split additions/deletions. | Use **Entry stats**, **Section stats**, **Known text stats**, or **Unknown stats**. |
...
| divider | Could mean the repository context frame or a section-level separator. | Use **Border line** for the repository context frame; use **Section divider** for the line below a Section header. |
| slash | Could mean punctuation, path separator, or stats separator. | Use **Stats separator** for the `/` in Entry stats or Section stats; Branch commit stats use a space, not `/`. |
