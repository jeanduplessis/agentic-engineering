...
- Recognize help/version/`--quiet`/`-q`/`--` before the first message word. Reject other leading options; preserve all words after the first message word. Join arguments with single spaces without trimming their contents. Reject empty/whitespace-only or non-Unicode requests before launching Pi.
- Supervise `pi --mode json -- <one prompt>` directly with `Command`. Never interpolate through a shell. Use `serde_json` for protocol parsing, `signal-hook` and `libc` for bounded Unix supervision; keep dependencies small and locked.
...
- Do not add model/backend configuration, flag passthrough, approval bypasses, a confirmation gate, sandbox claims, session management, verbose/log UI, or another runtime. Prompt guidance is not an enforced safeguard.
