...
| `tools.skill_valid` | End-to-end validity gate for one repo-local skill. | `python3 -m tools.skill_valid skills/<skill-name> --allow-live-pi`; `validate_skill(...)` | Orchestrates target, manifest, AGENTS.md, `llm_optimal_check`, live opt-in, validate-skills, and live eval gates. Stdout is compact JSON. |
| `tools/skill_eval/skill_eval.sh` | Friendly human wrapper around `tools.skill_valid`. | `./tools/skill_eval/skill_eval.sh skills/<skill-name>` | Renders gate statuses, optimization findings, raw JSON option, and preserved failure artifact paths. |
...
- Need to decide whether a repo-local skill is valid: use `tools.skill_valid` or the friendly `skill_eval.sh` wrapper.
...
python3 -m unittest tools.skill_eval.tests.test_skill_eval_wrapper -v
