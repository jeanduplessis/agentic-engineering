...
- On Pi upgrades, verify the exported `compact()` signature and the registered provider's `streamSimple()` contract. Assert that summary requests expose no tools, not a fixed native `toolChoice` value. Retain the upstream MIT notice and update README compatibility notes and root changelog when behavior changes.
